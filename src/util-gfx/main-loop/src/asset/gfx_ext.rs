...
pub trait PipelineLayoutConfigs<K: PipelineSet>: Sized {
    fn load(self, assets: &mut AssetManager, gfx: &GfxContext) -> Asset<PipelineLayout<K>>;
}

pub trait PipelineLayoutConfigsDefault: PipelineSet {
    fn load_default(assets: &mut AssetManager, gfx: &GfxContext) -> Asset<PipelineLayout<Self>>;
}

macro_rules! impl_pipeline_layout_configs {
	($($para:ident:$field:tt),*) => {
		impl<$($para: 'static + BindGroupExt),*> PipelineLayoutConfigs<($($para,)*)> for ($($para::Config,)*) {
			fn load(self, assets: &mut AssetManager, gfx: &GfxContext) -> Asset<PipelineLayout<($($para,)*)>> {
...
pub trait PipelineLayoutExt: Sized {
    ...
    fn load_default(assets: &mut AssetManager, gfx: &GfxContext) -> Asset<Self>
    where
