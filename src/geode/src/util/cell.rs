...
/// An [UnsafeCell] that asserts that its users are properly synchronizing access to its contents
/// across threads. This cell makes no assertion that the returned immutable references will be used
/// in a thread-safe manner so `T: Sync` in order for `SyncUnsafeCell<T>: Sync`. If you know you only
/// need a mutable reference to the contents, you can use [SyncUnsafeCellMut]—which will always be
/// `Sync`.
...
pub struct UsuallySafeCell<T: ?Sized>(SyncUnsafeCell<T>);
