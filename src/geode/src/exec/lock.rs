...
mod internals {
	use crate::exec::event::EventTarget;
	...
	impl Debug for RwLockManager {
		fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
			f.debug_struct("RwLockManager")
...
pub use crate::util::lifetime::{
	LimitLifetimeMut as RwResultMut, LimitLifetimeRef as RwResultRef, NoCopyRef as RwRefWrapper,
};
...
unsafe impl<'a, T: ?Sized> LockTarget for RwRefWrapper<'a, T> {
	type Deps = &'a RwLock<T>;

	unsafe fn get(deps: &Self::Deps) -> Self {
		RwRefWrapper::new(&*(deps.value.get()))
	}
...
impl<T> RwLock<T> {
	pub fn new(manager: RwLockManager, value: T) -> Self {
...
impl<T: ?Sized + Debug> Debug for RwLock<T> {
	fn fmt(&self, f: &mut Formatter<'_>) -> FmtResult {
		...
		if let Some(guard) = self.try_lock_ref_now() {
			let value: &T = &**guard.get_ref();
			builder.field("value", &value);
		} else {
			builder.field("value", &"<would block>");
		}
...
impl<T: ?Sized> RwLock<T> {
	...
	pub fn lock_ref_async_or_fail(&self) -> RwLockFuture<RwRefWrapper<T>> {
		RwLockFuture::new(self)
	}
...
pub trait RwLockExt: Sized {
	fn wrap_lock<M: Take<RwLockManager>>(self, manager: M) -> RwLock<Self> {
		RwLock::new(manager.take_owned(), self)
	}
}
...
pub type RwGuardRef<'a, T> = RwGuard<RwRefWrapper<'a, T>>;
...
impl<T: Debug + LockTarget> Debug for RwGuard<T> {
	fn fmt(&self, f: &mut Formatter<'_>) -> FmtResult {
		f.debug_struct("RwLock")
			.field("values", &*self.get_ref())
...
impl<T: LockTarget> RwGuard<T> {
	...
	pub fn get(&mut self) -> RwResultMut<'_, T> {
		RwResultMut::new(unsafe { T::get(&self.deps) })
	}

	pub fn get_ref(&self) -> RwResultRef<'_, T> {
		RwResultRef::new(unsafe { T::get(&self.deps) })
	}
}
