...
pub trait PointerLike<'a> {
	...
	type AsRef: Clone + Deref<Target = Self::Value>;

	type AsMut: Deref<Target = Self::Value> + DerefMut;
...
impl<'a, T: ?Sized + 'a> PointerLike<'a> for NonNull<T> {
	...
	unsafe fn promote_mut(mut self) -> Self::AsMut {
		self.as_mut()
	}
}
...
pub type RefOf<'a, T> = <PtrOf<T> as PointerLike<'a>>::AsRef;
pub type MutOf<'a, T> = <PtrOf<T> as PointerLike<'a>>::AsMut;
...
/// guarantees about objects implementing this trait is through [Promise]s such as [PromiseUnborrowed].
pub trait Accessor {
	type Index: Debug + Copy + Eq;
	type Value: ?Sized;

	// TODO: Use HRTB lifetime constraints once they stabilize to allow accessing of non-'static data.
	type Ptr: for<'a> PointerLike<'a>;
	type OobError: Error;
...
// TODO: Add support for accessor stubs, which would allow certain accessor-derived references to
//  outlive their parent accessor.
...
impl<'a, T: 'static> AsAccessor<'a> for [T] {
...
impl<'a, T: 'static> AsAccessorMut<'a> for [T] {
...
impl<'a, T: 'static> Accessor for SliceRefAccessor<'a, T> {
	type Index = usize;
	type Value = T;
	type Ptr = NonNull<T>;
...
impl<'a, T: 'static> Accessor for SliceMutAccessor<'a, T> {
	type Index = usize;
	type Value = T;
	type Ptr = NonNull<T>;
	type OobError = SliceOobError;
...
where
	...
	I: Iterator<Item = A::Index>,
{
	type Item = <A::Ptr as PointerLike<'a>>::AsMut;
	...
	fn next(&mut self) -> Option<Self::Item> {
		...
		Some(unsafe { self.accessor.get_raw(index).promote_mut() })
	}
}
