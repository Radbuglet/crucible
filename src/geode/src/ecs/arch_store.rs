use crate::ecs::map_store::MapStorage;
use crate::ecs::world::{ArchHandle, DirtyId, StorageId, World};
use crate::util::free_list::IterableFreeList;
use std::cell::UnsafeCell;
use std::mem::MaybeUninit;

pub struct ArchStorage<T> {
	id: StorageId,
	last_checked: DirtyId,
	locs: MapStorage<EntityLoc>,
	archetypes: IterableFreeList<StorageArchetype<T>>,
}

#[derive(Debug)]
struct StorageArchetype<T> {
	handle: ArchHandle,
	components: Vec<UnsafeCell<MaybeUninit<T>>>,
}

#[derive(Debug, Copy, Clone, Hash, Eq, PartialEq)]
struct EntityLoc {
	arch_index: usize,
	comp_index: usize,
}

impl<T> ArchStorage<T> {
	pub fn flush(&mut self, world: &World) {
		//> The objective of this routine is to ensure that the layout of the world's template
		//> archetypes matches those of this storage.

		// For every archetype that might have slots that need to be updated with a new target
		// component...
		// FIXME: This should handle archetypes that haven't been created yet.
		for (store_index, store_arch) in self.archetypes.iter() {
			// Find the template archetype for this local archetype.
			let template_arch = world.get_archetype(store_arch.handle).unwrap();

			// Ensure that the archetype is big enough to accommodate the components being moved
			// into it.
			// TODO

			// TODO: Justify why things can't crash.

			// For every slot that should be a potentially different entity than when we last flushed
			// the storage...
			let dirty_iter = template_arch.iter_dirties(self.last_checked);
			for (target_dirty_id, target_slot) in dirty_iter {
				// Find the swap source.
				let swap_source = store_arch.components[target_slot].get();

				// Figure out what the slot should be.
				// (this cannot fail because only extant slots can have a `should_be` entry)
				let should_be_entity = template_arch.entities()[target_slot];

				//> Figure out where the `should_be_entity` currently resides.
				// The outer loop maintains the invariant that a given entity in the storage that is
				// not associated explicitly with its target node can always be found by following
				// the chain of `should_be_at` references with respect to an untouched `locs` map,
				// starting with the slot corresponding to `should_be_entity` and ending with the
				// first slot that hasn't been updated by the loop yet.
				//
				// The base case, which is when the routine begins, is proven below:
				// Initially, no entities have been updated and the `locs` map points directly to
				// every entity. Thus, the search procedure works correctly for all of them, proving
				// the base case.
				//
				// The `n -> n+1` case is proven in the swap routine.

				// Figure out the entity's original position before we started flushing.
				// This entity is guaranteed to have not yet been explicitly associated with its
				// target node as this slot is still dirty.
				let mut should_be_at = *self.locs.get_raw(should_be_entity);

				// This routine implements the search routine described above, which we assume
				// functions properly because of an assumed invariant.
				let swap_dest = loop {
					let should_be_store_arch = &self.archetypes[should_be_at.arch_index];
					let should_be_template_arch = world.get_archetype(should_be_store_arch.handle);

					match should_be_template_arch {
						// If we already processed the slot, it is non-terminal.
						#[rustfmt::skip]
						Ok(arch)
							if should_be_at.comp_index < arch.entities().len() && (
								should_be_at.arch_index < store_index ||
								arch.dirty_version_of(should_be_at.comp_index) > target_dirty_id
							)
						=> {
							should_be_at = *self.locs.get_raw(arch.entities()[should_be_at.comp_index]);
						}
						// Otherwise, we found a slot that hasn't yet been finalized. This must be
						// the source slot.
						_ => {
							break should_be_store_arch.components[should_be_at.comp_index].get();
						}
					};
				};

				// `std::ptr::swap` is a bit different from this pattern in that regions are allowed
				// to partially overlap, whereas this pattern ensures that elements either overlap
				// entire or don't overlap at all. This should hopefully help codegen generate more
				// efficient code.
				if swap_source != swap_dest {
					unsafe { std::ptr::swap_nonoverlapping(swap_source, swap_dest, 1) };
				}

				// Having now processed and swapped slots in the storage, we must now prove that the
				// invariants have been upheld.
				//
				// There are three things that have changed here:
				// a) `target_slot` has been marked as having been updated
				// b) The component at `swap_source`, associated with an entity we'll call `S`, has
				//    been swapped with `swap_dest`, which is associated with an entity we'll call
				//    `D`.
				//
				// Because `D` is now associated with its target slot, no additional considerations
				// need to be made for it. Because `S` was guaranteed to be searchable from `S` by
				// the function's initial invariants, the target slot, and everything that came between
				// it and `S`'s original position, are continuation nodes. Because `D` itself is now
				// a continuation node, searching from the `S` origin to the new `S` terminator is
				// guaranteed to be allowed.
				//
				// TODO: How do we phrase that in an actually comprehensible way?
			}
		}

		//> Re-associate the location map with the entries' current position in the storage.
		// Entities not associated with anything were either cleaned up by an explicit remove command,
		// or were de-spawned, a situation `MapStorage` handles automatically.
		for (store_index, store_arch) in self.archetypes.iter_mut() {
			// Find the template archetype for this local archetype.
			let template_arch = world.get_archetype(store_arch.handle).unwrap();

			// Relocate each slot
			let dirty_iter = template_arch.iter_dirties(self.last_checked);
			for (_, slot) in dirty_iter {
				self.locs.insert(
					world,
					template_arch.entities()[slot],
					EntityLoc {
						arch_index: store_index,
						comp_index: slot,
					},
				);
			}
		}

		self.last_checked = world.latest_dirty_id();

		//> Run destructors
		// TODO
	}
}
