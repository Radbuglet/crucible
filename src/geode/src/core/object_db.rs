...
mod db {
	...
	#[derive(Default)]
	pub(crate) struct SlotManagerState {
		free_slots: Vec<&'static Slot>,
		slot_alloc: LeakyBump,
		gen_alloc: LocalBatchAllocator,
	}

	impl StaticStorageHandler for SlotManagerState {
	...
	pub fn acquire_slot(
		...
		let state = unsafe { SlotManagerState::get(session).get_mut_unchecked() };
		...
		#[cold]
		#[inline(never)]
		fn allocate_new_slot(state: &mut SlotManagerState) -> &'static Slot {
...
pub(crate) use db::SlotManagerState;
use thiserror::Error;
