...
fn main() {
	...
	for entry in glob("src/render/shader/**/*.vert").expect("Failed to glob `.vert` files") {
		build_shader(
			&mut compiler,
	...
	for entry in glob("src/render/shader/**/*.frag").expect("Failed to glob `.frag` files") {
		build_shader(
			&mut compiler,
			entry.expect("Failed to glob specific file."),
			ShaderKind::Fragment,
...
fn build_shader(compiler: &mut Compiler, mut path: PathBuf, kind: ShaderKind) {
	...
	let artifact = match compiler.compile_into_spirv(
		&*fs::read_to_string(&path).expect(
			format!(
				"Failed to read render.shader source at {:?}",
				path.as_path()
			)
			.as_str(),
		),
