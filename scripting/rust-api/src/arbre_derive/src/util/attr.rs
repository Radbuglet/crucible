...
use syn::{AttrStyle, Attribute, Error as SynError, Path, Result as SynResult};
...
pub fn parse_attrs_subset<'a, A>(
    attrs: &'a Vec<Attribute>,
    supported: &[A],
) -> SynResult<impl Iterator<Item = (A, &'a Attribute)>>
where
    A: MetaEnum<Meta = HelperAttr>,
{
        ...
        match attr.style {
            ...
            _ => {
                return Err(SynError::new(
                    attr.span(),
                    format!(
                        "`{}` must be an outer attribute in this context.",
                        key.meta().name
                    ),
                ))
            }
...
pub fn parse_attrs_single<A>(
    attrs: &Vec<Attribute>,
    supported: A,
) -> SynResult<Option<(A, &Attribute)>>
where
    A: MetaEnum<Meta = HelperAttr>,
{
    ...
    if let Some((key, duplicate)) = attrs.next() {
        return Err(SynError::new(
            duplicate.span(),
            format!("`{}` can only show up once per item.", key.meta().name),
        ));
    }
    Ok(first)
}
